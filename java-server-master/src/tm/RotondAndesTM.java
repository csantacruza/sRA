...
public class RotondAndesTM {
	...
	/**
	 * Metodo que modela la creación de un pedido con susrespectivas equivalencias.
	 * @param Pedido
	 * @param idCliente
	 * @throws Exception
	 */
	...
	public void addPedidoEquivalencias(Pedido Pedido, Integer idCliente) throws Exception {
		...
		try 
		{
			...
			if(buscarMenuPorNombre(Pedido.getNombreProducto()) != null && buscarMenuPorNombre(Pedido.getNombreProducto()).getNombre() != "") {
				...
				//Es un pedido de un menu orginal
				if(Pedido.getComponentes().isEmpty()) {
					daoPedidos.addPedido(Pedido,idCliente);
					conn.commit();		
				}
				//Es un pedido de un menu con equivalencias en los productos
				else {
					...
					for(int j =  0; j < Pedido.getComponentes().size(); j ++) {
						...
						for( int i = 0; i < productos.size();i++) {
							if(!parte[0].equals(productos.get(i))) {
								count++ ;
							}
						}
					...
					for(int x =  0; x < Pedido.getComponentes().size(); x ++) {
						...
						int sum = 0;
						equiv = buscarEquivalenciasProducto(parte[0],Pedido.getNombreRestaurante());
						this.conn = darConexion();
						daoPedidos.setConn(conn);
			...
			//Si es un producto
				...
				//Es un pedido de un producto con equivalencias en los ingredientes
				else {
					...
					for(int j =  0; j < Pedido.getComponentes().size(); j ++) {
						...
						for( int i = 0; i < ingredientes.size();i++) {
							if(!parte[0].equals(ingredientes.get(i))) {
								count++ ;
							}
						}
					...
					for(int x =  0; x < Pedido.getComponentes().size(); x ++) {
						String[] parte = Pedido.getComponentes().get(x).split(",");
						int sum = 0;
						equiv = buscarEquivalenciasIngrediente(parte[0],Pedido.getNombreRestaurante());
						this.conn = darConexion();
						daoPedidos.setConn(conn);
						for( int i = 0; i < equiv.size();i++) {
						...
						if(sum == equiv.size()) {
							...
							daoPedidos.addPedidoCompProducto(Pedido,idCliente);
						}
	...
	public void consultarConsumoV1Administrador(ConsultarConsumo cC) throws Exception{
		DAOConsultarConsumo daoConsultarConsumo = new DAOConsultarConsumo();
		try 
		{
			//////transaccion
